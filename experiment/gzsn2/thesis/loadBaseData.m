function [baseOutputs, rawData, labels, screeningLabels, screenedObjs, objIds, agentIds] = ...
...
[agentIds, ~, agentIdxs] = unique(agents); 
...
%     objIds = [objIds; sepNegLabs(~ismember(sepNegLabs,objIds))]; 
    ...
    display(['Number of conflicting true labels: ' num2str(sum(ismember(objIds, sepPosLabs) & ismember(objIds, sepNegLabs)))]);
...
%     
%     noTypeIdxs = ismember(objIdxs, find(noTypes));
%     noTypeNegScores = sparse(1, objIdxs(noTypeIdxs), double(scores(noTypeIdxs)==-1));
%     noTypePosScores = sparse(1, objIdxs(noTypeIdxs), double(scores(noTypeIdxs)~=-1));
%     noTypeNegScores = noTypeNegScores>10 & ~noTypePosScores>0;

%     labels(noTypeNegScores) = 1;
%     labels(noTypes) = -2;
%     noTypeSamples = find(labels(objIdxs)==-2);
%     noTypeScores = scores(noTypeSamples);
%     ignoreIdxs = noTypeScores ~= -1;
%     ignoreIdxs = objIdxs(noTypeSamples(ignoreIdxs));
%     
%     labels(ignoreIdxs) = 0;    
%     
%     
