...
for f=1:nRounds
    ...
    lam_knockon_f = (log(fpr./(1-fpr)) + log(fnr./(1-fnr)))./2;
    lam_knockon_f = exp(lam_knockon_f) ./ (1+exp(lam_knockon_f))
    lam_knockon(f) = lam_knockon_f;
%     
...
%     save('/homes/49/edwin/matlab/combination/data/active_label/cyber/knockonIG_3req.mat','aucs_knockon1');
    ...
    samplesToTry = trainIdxs(leastCertIdx(1:nToTry));
    ...
    for i=1:length(samplesToTry)
        ...
        otherUnlabTrainIdxs = trainIdxs(trainIdxs~=s);
