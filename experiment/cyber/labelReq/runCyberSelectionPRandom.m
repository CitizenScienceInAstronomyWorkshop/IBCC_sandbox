...
for f=1:nRounds
    ...
    fpr_rand_f = (log(fpr./(1-fpr)) + log(fnr./(1-fnr)))./2;
    fpr_rand_f = exp(fpr_rand_f) ./ (1+exp(fpr_rand_f))
    fpr_rand3(f) = fpr_rand_f;
...
%     aucs(:, f) = auc;
    ...
    for r=1:nRequests
        leastCertIdx = randi(length(unlabTrainIdxs),1);
