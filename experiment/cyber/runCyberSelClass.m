%selects the best feature/object pairs to maximise the IG for each class,
%i.e. selects 8 at a time
...
subSampleSize = 50; subSampleNAgents = 100;
...
for f=1:300
    ...
    currentBaseData(Ass'==0) = -1;
    runner.basePost = currentBaseData;
    ...
    for j=1:runner.nClasses
        Alphaj = zeros(2, size(Alpha,2), size(Alpha,3));
        Alphaj(1, :, :) = Alpha(j, :, :);
        Alphaj(2, :, :) = sum(Alpha, 1) -  Alpha(j, :, :);
        Nuj = [Nu(j) sum(Nu)-Nu(j)];
        ETj = [ET(j,:); 1-ET(j,:)]; 
        
        state = {Alphaj, Nuj, ETj, state{4}};    
        
        [chosenTask, chosenUser, IG] = greedyInfoGain(state, N, K, A, 1);
        idxs = sub2ind(size(state{4}), chosenTask, chosenUser);
        state{4}(idxs) = state{4}(idxs)+1;
        display(['Number of assignments: ', num2str(sum(sum(state{4}))) ]);
    end
end
