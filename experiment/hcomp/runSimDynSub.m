...
% featureFile = [rootDir '/matrices_2000_topic.txt']; %complete TREC dataset
...
while length(selectedDocs)<batchSize
    selectedDocs = unique([selectedDocs negEgs(1:batchSize-length(selectedDocs))' ]);
end
...
Alpha0 = 1.5 .* ones(nClasses, 2); %0.5 
...
%         nToAllocate = nAllocateQueue(1);
        ...
        while newWorkersNeeded > 0
            ...
            skills = [0.4 0.65 0.9]; %hcomp sf4 has 0.4 moved to the end
