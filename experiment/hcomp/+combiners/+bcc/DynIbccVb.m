...
%                 P_tminus(:,:,:) = P_tminus(:,:,:) + repmat(reshape(obj.settings.changeRateMod*q_t, [1,1,obj.nScores]), obj.nClasses, obj.nClasses);
                ...
                r_tminus = h' * reshape(h'*P_tminus_r, obj.nClasses, obj.nScores);                
...
%                     obj.q_t(:,n) = obj.q_t(:,n) + labelUncertainty;                    
                ...
                r_tpost(n,corrections) = r_tminus(corrections);
                
                tmp = reshape(P_tminus(:,:,:), obj.nClasses, obj.nClasses*obj.nScores)' * h;
                K_kal = reshape(tmp, [obj.nClasses, obj.nScores]);
                K_kal = K_kal ./ repmat(r_tminus, obj.nClasses, 1);
                obj.K_kal(:,:,n) = K_kal;
                
                R(n,:) = 1-(r_tpost(n,:)./r_tminus);
                Wmean_post(:,:,n) = Wmean_tminus + K_kal.*...
                    repmat((eta_tpost(n,:) - eta_tminus), obj.nClasses,1);

                Kh = reshape(K_kal, 1,obj.nClasses*obj.nScores)' * h';
