classdef  WeightedSum < combiners.weighted.WeightedCombiner
    ...
    methods
        ...
        function [invW] = updateAndCombine(obj, n, scores, post, T)
            invW = obj.weightCalculator.calculateWeights(scores, post, T, obj.combinedPost, n);
...
%                     contributions(currentAgent, expiredIdx) = 0;
        ...
        function combinedPost = combineCluster(obj, post, T, members, k)        
            ...
            invW = obj.updateAndCombine(1, scores, post, Trep);
            n=2;
            while n <= nTargets
                if n < startIdx(1)
                    obj.combinationStep(post, invW(:,end), n, startIdx(1)-1); 
                    n = startIdx(1);
                elseif n==startIdx(1);
                    invW = obj.updateAndCombine(n, scores, post, Trep);
                    n = n+1;
                    nLearnt = nLearnt+1;
        ...
        function createWeightCalculator(obj, members, nSamples)
            ...
            obj.weightCalculator = combiners.weighted.weights.CombinedErrorWeights(obj.nAgents, 100, members, nSamples);
        end
...
%             weightedVotes = nW' * post;
            ...
            %if some base classifiers do not give a score they are ignored.
            combinedPost_n = obj.combineDataPoints(post, 1./invW);%weightedVotes ./ totalWeights;
