...
% featureFile = [rootDir '/matrices_2000_topic_thresholded.txt']; %complete TREC dataset. Thresholded features with value < 0.01 for speed (around half all entries). Also seem to have cocked up the higher values?
% featureFile = [rootDir '/matrices_2000_topic.txt']; 
featureFile = [rootDir '/matrices_2000_topic_norm.txt']; %normalised so that max value is 1 for each feature
...
% labelFile = '/home/edwin/data/aaai/trec_real_final/crowd_class_labels7.csv';
labelFile = '/home/edwin/data/aaai/trec_real_final/crowd_labels_targets.csv';
...
Nu0 = repmat(100, 1, nClasses); Nu0(2:end) = 1;
Nu0ROC = repmat(100, 1, nClasses); Nu0ROC(2:end) = 1;
...
crowdTrust( sub2ind(size(crowdTrust), 1:nClasses, 1:nClasses) ) = 1.5;%3;
% crowdTrust(:,1) = 3;
% crowdTrust(1,2:end) = (3+nClasses-2)/(nClasses-1);
% crowdTrust = (crowdTrust./2) + 3;
...
Alpha0(:,2) = 0.5;%500;%990;%0.25;%0.25; %0.25
...
%         nToAllocate = nAllocateQueue(1);
        ...
        while newWorkersNeeded > 0
            ...
            skills = [1/nClasses^0.7 1/nClasses^0.33 1/nClasses^0.07]; %hcomp sf4 has 0.4 moved to the end
...
%         chosenIdx, crowdTrust, Alpha0, Nu0);
    ...
    [~, ~, ~, P, combiner, ~,~,logJoint] = classifyAndSelectSub( selectMethod, 0, 0, 0, ...
        featureFile, labelFile, selectedDocsFile,...
        excludedWorkersFile, centroidsFile, outputRequestFile, excludedWorkersFile,...
        chosenIdx, crowdTrustROC, Alpha0ROC, Nu0ROC);    
