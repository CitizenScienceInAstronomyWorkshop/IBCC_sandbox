classdef ClassifierPerformanceGraph < handle
    ...
    methods (Static)
        ...
        % ROC CURVE
        function [auc,fig,thresholds, lineHandles] = drawRoc(labelledCombinedPost, ...
                knownLabels, combMethods, separateMethods, aucOnly, newFigure, noLegend)
            if ~exist('separateMethods','var')
                separateMethods = false;
            end
            
            if ~exist('aucOnly','var')
                aucOnly = false;
            end
            ...
            for j=0:nClasses-1
                ...
                for c=1:length(combMethods)
                    ...
                    if nargin < 5 || ~aucOnly
                        lineHandles = [lineHandles plot(fpr_c, tpr_c, 'linewidth', 2, 'clipping', 'off')];
                        hold all;
                    
                        if separateMethods && ~noLegend
                            legend(combMethods{c}, 'Location', 'Best');
                        end
                    end
                end
                if nargin < 5 || ~aucOnly
                    ...
                    if ~separateMethods && ~noLegend
                        legend(combMethods, 'Location', 'Best');            
                    end
                end
                if fig~=0           
                    grid on
