...
graphMaker.label = '(a)';%sprintf('Mean Base Classifier Error for each Dataset');

meanErrors = reshape(meanErrors, 1, nAgents*expSettings.nDatasets);
errorVars = reshape(errorVars, 1, nAgents*expSettings.nDatasets);
baseErrors = reshape(baseErrors, 1, nAgents*nSamples*expSettings.nDatasets);

%subplot(2, 1, 1);
graphMaker.drawErrorHist(meanErrors, 0.02, true);
ylabel('No. Base Classifiers');

graphMaker.label = '(b)';%sprintf('Errors for Individual Data Points for all Datasets and Base Classifiers');
%subplot(2, 1, 2);
%graphMaker.drawErrorHist(baseErrors, 0.02, true);
%ylabel('No. Base Classifier Outputs');
