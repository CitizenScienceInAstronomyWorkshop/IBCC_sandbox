function [samplesToRequest, workersToRequest, excludedWorkers, Pall, combiner, ...
    ...
    crowdTrust, Alpha0, Nu0, scoreFile, unknownFile, alphaFile )
...
if nToAllocate>0
    [excludedWorkers, samplesToRequest, workersToRequest, latestAlpha, agentScores, unknownScores] = ...
...
if exist('scoreFile','var') && exist('agentScores','var')
    
    col1 = workerToAllocate;
    ...
    col3 = ones(length(agentScores),1) .* size(crowdLabels,1);
    
    cols = [col1 col2 col3];
    
    dlmwrite(scoreFile, cols, '-append');
end

if exist('unknownFile','var') && exist('unknownScores','var')
    ...
    dlmwrite(unknownFile, cols, '-append');
end

if exist('alphaFile','var') && exist('latestAlpha','var')
    
    if exist('alphaFile','file')
        load(alphaFile);
    else
        alphaRecord = zeros(size(latestAlpha,1), size(latestAlpha,2), 0);
    end
    
    cat(3, alphaRecord, latestAlpha);    
end